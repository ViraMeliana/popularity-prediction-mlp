...
class Testing:
    ...
    def pickle_parser(self):
        if os.path.exists("resources/models/vectorized.pickle") & os.path.exists(
                "resources/models/model_mlp_library.pickle"):
            print("model is exist")
            self.vectorized = pickle.load(open("resources/models/vectorized.pickle", 'rb'))
            self.model = pickle.load(open("resources/models/model_mlp_library.pickle", 'rb'))
        else:
    ...
    def predict(self, title):
        if self.model is not None:
            test_data_features, title_clean = self.title_cleaner(title)
            predict = self.model.predict(test_data_features)

            return predict[0], title_clean[1]
        else:
            return None
